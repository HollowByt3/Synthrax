...
run the following commands:
    ...
    ##This places the config in the correct directory so rsyslog can pipe the programs to the program
       sudo mv 60-synthrax-gateway.conf /etc/rsyslog.d/60-synthrax-gateway.conf


     Sudo sv start rsyslogd (runit system)
     systemctl start rsyslogd (systemd)
